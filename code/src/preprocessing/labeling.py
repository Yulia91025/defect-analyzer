    ...
    for phase in ['R', 'S', 'T']:
        ...
        try:
            ...
            features.append([
                ...
                peak_val, crest_factor, kurt_env
            ])
            labels.append([
...
def main():
    ...
    labels_df = pd.DataFrame(raw_labels, columns=[
        'outer_race', 'inner_race', 'rolling_element', 
        'cage', 'unbalance', 'misalignment'
    ])
    
    # Сохранение сырых меток для анализа
    raw_labels_df = pd.DataFrame(raw_labels, columns=[
        'outer_race', 'inner_race', 'rolling_element', 
        'cage', 'unbalance', 'misalignment'
    ])
    ...
    features_df = pd.DataFrame(all_features, columns=[
        'mean', 'std', 'skew', 'kurtosis', 'peak', 'crest_factor', 'kurtosis_env'
    ])
